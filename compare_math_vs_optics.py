...
def main():
    print("Initializing Continuous Optical Simulation...")
    ...
    # Fabricate Masks
    M1_img = np.zeros((size_mask, size_mask), dtype=np.float32)
    for i in range(8):
        for j in range(8):
            led_idx = i * 8 + j
    ...
    M2_img = np.zeros((size_mask, size_mask), dtype=np.float32)
    for i in range(4):
        for j in range(8):
            led_idx = i * 8 + j
    ...
    dx_ray = 0.05
    num_rays_1d = int(24.0 / dx_ray)
    
    opt_out1 = np.zeros(64)
    print("Tracing millions of optical rays through Layer 1...")
    ...
    for i in range(8):
        for j in range(8):
            ...
            X_p, Y_p = np.meshgrid(np.linspace(-12.0, 12.0, num_rays_1d), np.linspace(-12.0, 12.0, num_rays_1d))
            X_p = X_p.flatten()
            Y_p = Y_p.flatten()
            ...
            transmission = np.zeros_like(X_p)
    ...
    # Multiply by a normalization constant to bring light units to math units
    # (Since ray-casting is an unnormalized sum, we normalize both matrices below to their max value)

    # Electronic subtraction with bias
    pos_signals_o = opt_out1[:32]
    neg_signals_o = opt_out1[32:]
    opt_diff = pos_signals_o - neg_signals_o
    # Because optical sum is huge (millions of rays), we scale bias1 appropriately
    # We will normalize everything later, so for now we just do:
    # Actually, the simplest is to normalize out1 first, THEN subtract bias.
    # We will scale opt_out1 to match math_out1.
    scale_factor1 = np.max(math_out1) / (np.max(opt_out1) if np.max(opt_out1) > 0 else 1)
    opt_out1_scaled = opt_out1 * scale_factor1
    
    pos_signals_os = opt_out1_scaled[:32]
    neg_signals_os = opt_out1_scaled[32:]
    opt_diff_scaled = pos_signals_os - neg_signals_os + (bias1 / W1_max)
    opt_hidden = np.maximum(0, opt_diff_scaled)

    opt_out2 = np.zeros(10)
    print("Tracing optical rays through Layer 2...")
    for i in range(4):
        for j in range(8):
            led_idx = i * 8 + j
            ...
            X_p, Y_p = np.meshgrid(np.linspace(-8.0, 8.0, 200), np.linspace(-4.0, 4.0, 100))
            X_p = X_p.flatten()
            Y_p = Y_p.flatten()
    ...
    # Normalizing for visual plotting so both share the same colormap
    ...
    fig.suptitle(f"Continuous Optical MVM Simulation | Math Pred: {pred_math} | Optics Pred: {pred_opt} | True: {label}", fontsize=20, color='white')
    ...
    axes[1,0].set_title("Input LED Intensities", fontsize=14, color='lime')
    axes[1,0].set_ylabel("Optical\nSimulation", fontsize=16, fontweight='bold', color='lime')
    annotate_heatmap(axes[1,0], X, (8, 8))
    
    axes[1,1].imshow(opt_out1_scaled.reshape(8, 8), cmap='hot', interpolation='nearest')
    axes[1,1].set_title("Layer 1 PD Irradiance", fontsize=14, color='lime')
    annotate_heatmap(axes[1,1], opt_out1_scaled, (8, 8))
    
    axes[1,2].imshow(opt_hidden.reshape(4, 8), cmap='hot', interpolation='nearest')
    axes[1,2].set_title("Hidden Layer LED Intensities", fontsize=14, color='lime')
    annotate_heatmap(axes[1,2], opt_hidden, (4, 8))
    
    axes[1,3].imshow(opt_final.reshape(2, 5), cmap='hot', interpolation='nearest')
    axes[1,3].set_title(f"Output PD Signals", fontsize=14, color='lime')
    ...
    plt.tight_layout()
    plt.savefig('C:/Users/HP/.gemini/antigravity-ide/brain/e6af67b9-21b7-4c7c-b7bf-c42a371e207b/compare_math_optics.png', bbox_inches='tight')
    print("Saved Continuous Optics comparison to compare_math_optics.png")
